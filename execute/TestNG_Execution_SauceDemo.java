...
public class TestNG_Execution_SauceDemo extends BaseClas {

	@Parameters({"browser"})
	@Test(priority = -900)
	private void browser(String browse) {
		browserLaunch(browse);
	...
	@Parameters({"user"})
	@Test(priority = -800)
	private void login(String userr) throws IOException {

		LoginPage l = new LoginPage();

		WebElement userName = l.getUserName();
		sendText(userName, userr);
	...
	private void addtocarts() {
		...
		boolean displayed = pro.getValidate_login().isDisplayed();
	...
	private void clickLogout() throws InterruptedException {
		...
		boolean displayed1 = log.getValidate_logout().isDisplayed();
